...
export default function ScreenerPage() {
  ...
  const toggleFavorite = (ticker: string) => {
    ...
    if (updated.includes(ticker)) {
      updated = updated.filter(t => t !== ticker)
    } else {
      updated.push(ticker)
    }
  ...
  // Fetch live BIST screener data on mount
  useEffect(() => {
    fetch("http://localhost:8000/api/v1/screener/")
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setCompanies(data)
          // Set selected ticker to first item if available
          if (data.length > 0) {
              ...
              <div className="overflow-x-auto max-h-[500px]">
                {loading ? (
                  <div className="flex flex-col items-center justify-center py-20 space-y-3">
                    ...
                    <span className="text-xs text-muted-foreground font-semibold">BIST Verileri Yükleniyor...</span>
                  </div>
                ) : (
                    ...
                    <tbody>
                      {sortedAndFilteredCompanies.length > 0 ? (
                        sortedAndFilteredCompanies.map((comp) => {
                          const isFav = favorites.includes(comp.ticker)
