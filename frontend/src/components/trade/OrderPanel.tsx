...
export default function OrderPanel() {
  ...
  if (!instrument) {
    return (
      <div className="h-40 xl:h-full flex flex-col items-center justify-center gap-2 bg-card border border-border rounded-xl text-muted-foreground/70">
  ...
  return (
    <div className="flex flex-col h-auto xl:h-full bg-card border border-border rounded-xl xl:overflow-y-auto">
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-sm font-black text-foreground">{instrument.symbol}</div>
              <div className="text-[10px] text-muted-foreground truncate max-w-[160px]">{instrument.name}</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-lg font-black text-foreground">{lastPrice.toFixed(2)}</div>
            <div className={`text-[10px] font-bold ${isUp ? "text-bull" : "text-bear"}`}>
        ...
        <div className="grid grid-cols-3 gap-2 mt-3 text-center">
          <div className="bg-secondary/60 rounded-lg py-1.5">
            <div className="text-[9px] text-muted-foreground uppercase">Alış</div>
            <div className="text-xs font-bold text-bull">{instrument.bid.toFixed(2)}</div>
          </div>
          <div className="bg-secondary/60 rounded-lg py-1.5">
            <div className="text-[9px] text-muted-foreground uppercase">Satış</div>
            <div className="text-xs font-bold text-bear">{instrument.ask.toFixed(2)}</div>
          </div>
          <div className="bg-secondary/60 rounded-lg py-1.5">
            <div className="text-[9px] text-muted-foreground uppercase">Spread</div>
            <div className="text-xs font-bold text-foreground">{spread.toFixed(2)}</div>
          </div>
        </div>
        ...
        {openPosition && (
          ...
          >
            <span className="flex items-center gap-1.5 text-muted-foreground">
              ...
              <span className="font-bold text-foreground">{openPosition.lot} lot</span>
            </span>
            <span className={openPosition.pnl >= 0 ? "text-bull font-bold" : "text-bear font-bold"}>
      ...
      <div className="p-4 space-y-3 flex-1">
        <div className="grid grid-cols-4 gap-1 bg-secondary border border-border rounded-lg p-1">
          {([
            ["MARKET", "Piyasa"],
            ["LIMIT", "Limit"],
            ["STOP", "Stop"],
            ...
            <button
              ...
              className={`h-8 rounded-md text-[10px] font-bold cursor-pointer transition-colors px-1 ${
                orderType === value ? "bg-primary text-background" : "text-muted-foreground hover:text-foreground"
              }`}
            >
              {label}
            </button>
        ...
        <div className={`grid gap-2 ${needsLimitInput && needsStopInput ? "grid-cols-3" : needsLimitInput || needsStopInput ? "grid-cols-2" : "grid-cols-1"}`}>
          <div>
            <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Lot</label>
            <input
              type="text"
              inputMode="decimal"
              value={lot}
              onChange={e => setLot(e.target.value)}
              className="w-full h-9 mt-1 px-3 rounded-lg bg-secondary border border-border text-sm font-bold text-foreground focus:outline-none focus:border-primary/40"
            />
          </div>
          {needsStopInput && (
            <div>
              <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Stop Fiyatı</label>
              <input
                type="text"
                inputMode="decimal"
                value={stopPrice}
          ...
          {needsLimitInput && (
            <div>
              <label className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">Limit Fiyatı</label>
              <input
                type="text"
                inputMode="decimal"
                value={limitPrice}
        ...
        <div className="rounded-xl border border-border bg-secondary/50 divide-y divide-border/60 overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 text-[11px]">
            <span className="text-muted-foreground">
              {needsLimitInput ? "Limit Fiyatı" : orderType === "STOP" ? "Stop Fiyatı (tahmini)" : "Fiyat"}
            </span>
            <span className="font-semibold text-foreground">{effectivePrice.toFixed(2)} ₺</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2 text-[11px]">
            <span className="text-muted-foreground">Toplam Tutar</span>
            <span className="font-semibold text-foreground">{notional.toFixed(2)} ₺</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2 text-[11px]">
            <span className="text-muted-foreground">Komisyon (tahmini)</span>
            <span className="font-semibold text-foreground/80">{commission.toFixed(2)} ₺</span>
          </div>
          <div className="flex items-center justify-between px-3 py-2.5 bg-secondary/80">
            <span className="text-[11px] font-bold text-foreground/80">Tahmini Toplam</span>
            <span className="font-black text-foreground text-sm">{total.toFixed(2)} ₺</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <button
            ...
            className="flex items-center justify-center gap-1.5 h-11 rounded-lg bg-bull hover:bg-bull/90 disabled:opacity-50 text-background font-black text-sm transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-1.5 h-11 rounded-lg bg-bear hover:bg-bear/90 disabled:opacity-50 text-white font-black text-sm transition-colors cursor-pointer"
          >
